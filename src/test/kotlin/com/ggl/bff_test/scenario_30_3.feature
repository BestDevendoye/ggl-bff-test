Feature:
  ...
  @ignore
  Scenario: scenario_30_3
    Then print 'addEntryToCart'
    Given def query = read(requestBaseFile + '/cart/add_entry/query.graphql')
    Given def vars = read(requestBaseFile + '/cart/add_entry/vars_scenario_30_3_article.json')
    ...
    * def accessToken = new KeycloakConnect().login().accessToken
    ...
    Given def vars = read(requestBaseFile + '/cart/add_customer/vars_scenario_30_3.json')
    ...
    * def accessToken = new KeycloakConnect().login().accessToken
    ...
    Given def vars = read(requestBaseFile + '/cart/update_sg/vars_scenario_30_3.json')
    And header Authorization = 'Bearer ' + accessToken
    And request { query: '#(query)', variables: '#(vars)' }
    When method post
    Then status 200
    ...
    * def accessToken = new KeycloakConnect().login().accessToken
    ...
    Given def vars = read(requestBaseFile + '/order/create/vars_scenario_30_3.json')
    And header Authorization = 'Bearer ' + accessToken
    And request { query: '#(query)', variables: '#(vars)' }
    When method post

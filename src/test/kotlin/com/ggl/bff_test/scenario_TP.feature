Feature:
  ...
  Scenario: scenario_TP
    ...
    * def accessToken = new KeycloakConnect().login().accessToken
    ...
    Given def vars = read(requestBaseFile + '/product/create/vars_scenario_TP.json')
    ...
    * def accessToken = new KeycloakConnect().login().accessToken
    ...
    And header karate-name = 'addDeliveryToCart'
    And header Authorization = 'Bearer ' + accessToken
    And request { query: '#(query)', variables: '#(vars)' }
    When method post
    Then status 200
    ...
    * def accessToken = new KeycloakConnect().login().accessToken
    ...
    Given def vars = read(requestBaseFile + '/order/create/vars_scenario_TP.json')
    And header karate-name = 'createOrder'
    And header Authorization = 'Bearer ' + accessToken
    And request { query: '#(query)', variables: '#(vars)' }
    When method post
    * match response.data == '#present'

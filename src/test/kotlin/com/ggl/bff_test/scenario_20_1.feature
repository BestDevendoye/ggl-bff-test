Feature:
  ...
  @ignore
  Scenario: scenario_20_1
    Then print 'addEntryToCart'
    Given def query = read(requestBaseFile + '/cart/add_entry/query.graphql')
    Given def vars = read(requestBaseFile + '/cart/add_entry/vars_scenario_20_1_article.json')
    ...
    * def accessToken = new KeycloakConnect().login().accessToken
    ...
    Given def vars = read(requestBaseFile + '/cart/add_customer/vars_scenario_20_1.json')
    ...
    * def accessToken = new KeycloakConnect().login().accessToken
    ...
    Given def vars = read(requestBaseFile + '/cart/update_sg/vars_scenario_20_1.json')
    And header Authorization = 'Bearer ' + accessToken
    And request { query: '#(query)', variables: '#(vars)' }
    When method post
    Then status 200
    ...
    * def accessToken = new KeycloakConnect().login().accessToken
    ...
    Given def vars = read(requestBaseFile + '/order/create/vars_scenario_20_1.json')
    And header Authorization = 'Bearer ' + accessToken
    And request { query: '#(query)', variables: '#(vars)' }
    When method post
